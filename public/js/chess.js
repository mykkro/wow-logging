var Board = joint.dia.Paper.extend({
	...
	if (!this.options.animation || opts.animation === false) {
	    ...
	    _.invoke(pc, 'transition', 'position', this._n2p(to), { valueFunction: joint.util.interpolate.object });
	}
    },
	...
	this.model.addCell(new joint.dia.Link({
	    ...
	    z: -1,
	    attrs: {
		'.': { opacity: .2, stroke: 'black' },
		'.marker-target': { d: "m 0, -10 a 10,10 0 1,0 20,0 a 10,10 0 1,0 -20,0", fill: 'black' },
		'.connection': { 'stroke-width': '4px' }
	    }
	}));
    },
	...
	_.chain(toArray)
	    .map(this._n2p, this)
	    .groupBy(function(p0) { return g.point(p0).theta(p1); })
	    .map(function(group) { return _.max(group, function(p0) { return g.point(p1).distance(p0); })})
    ...
    _p2n: function(p) { return this.options.letters[p.x / 50] + (8 - p.y / 50); },

    _n2p: function(n) {	return { x: this.options.letters.indexOf(n[0]) * 50, y: (8 - n[1]) * 50	}},

    _mid: function(p) { return { x: p.x + 25, y: p.y + 25 }},
...
var Chessboard = Board.extend({
	...
	if (mv & moveflagPromotion) {
	    ...
	    if (transition) this.listenToOnce(this.model, 'transition:end', promote); else promote();
	...
	var s = from + to;
	return _.find(GenerateValidMoves(), _.compose(function(m) { return m == s || m == s + 'q' }, FormatMove));
    },
	...
	return _.chain(GenerateValidMoves())
	    .map(FormatMove)
	    .filter(function(move) { return !move.lastIndexOf(from) })
	    .invoke('slice', 2, 4)
	    .value();
    },
...
board.on('message', function(rnd, mov, res) {
    document.getElementById('message').textContent +=(rnd % 2 ? '' : (1 + rnd / 2) + '. ') + mov + ' ' + res;
});
